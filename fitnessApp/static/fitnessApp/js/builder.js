...
function switch_quantity_rest() {
    for (i = 0; i < 5; i++) {
        for (j = 0; j < 15; j++) {
            if (document.getElementById("id-quantity-" + (i + 1) + "-" + (j + 1)).style.display == "none") {
                set_display_null("id-quantity-" + (i + 1) + "-" + (j + 1));
                set_display_none("id-rest-" + (i + 1) + "-" + (j + 1));
            }
            else {
                set_display_none("id-quantity-" + (i + 1) + "-" + (j + 1));
                set_display_null("id-rest-" + (i + 1) + "-" + (j + 1));
            }
        }
    }
}
...
function page_2_next() {
    ...
    if (form.checkValidity()) {
        ...
        var defaultReps = formData.get('exercise_reps');
        var defaultTime = formData.get('exercise_time');
        var defaultExerciseRest = formData.get('exercise_rest');
        var defaultSetRest = formData.get('set_rest');
        ...
        for (i = 0; i < 5; i++) {
            ...
            if (i < numCircuits) {
                ...
                for (j = 0; j < 15; j++) {
                    ...
                    var exerciseRestId = "id_exercise_rest_" + (i + 1) + "_" + (j + 1);
                    ...
                    var exerciseRestElement = document.getElementById(exerciseRestId);
                    ...
                    if (j + 1 < numberOfExercises)
                        exerciseRestElement.value = defaultExerciseRest;
                    else
                        exerciseRestElement.value = defaultSetRest;
                    ...
                    if (j < numberOfExercises) {
                        ...
                        exerciseRestElement.setAttribute('required', '');
                        if (!isNaN(exerciseItems[j])) {
                            exerciseNameElement.value = (j + 1) + ". " + exerciseOptions[exerciseItems[j]].name;
                        }
                        else {
                            exerciseNameElement.value = (j + 1) + ". " + "(New) " + exerciseItems[j];
                        }
                    }
                    else {
                        set_display_none(circuitExerciseId);
                        quantityElement.removeAttribute('required');
                        exerciseRestElement.removeAttribute('required');
...
function switchModeQuantity(mode, event) {
    ...
    if (mode == "REPS") {
        document.getElementById(event.target.dataset.quantityFieldId).value = defaultReps;
    }
    else {
        document.getElementById(event.target.dataset.quantityFieldId).value = defaultTime;
    }
}
...
function disableSubmitButton() {
    var btn = document.getElementById('id-builder-submit-button');
    btn.disabled = true;
    btn.innerText = 'Creating...';
}

var buildMode = "{{ buildMode }}";
var buildMode = JSON.parse(document.getElementById('id_build_mode').textContent);

if (buildMode == "SESSION") {
    var currentDate = new Date();
    document.getElementById('id-session-date').value = currentDate.getFullYear() + "-" + String(currentDate.getMonth() + 1).padStart(2, "0") + "-" + String(currentDate.getDate()).padStart(2, "0");
    document.getElementById('id-session-time').value = String((currentDate.getHours() + 1) % 24).padStart(2, "0") + ":00";
}
    ...
    new TomSelect(exercise,
        {
            ...
            onItemAdd: function () {
                ...
                this.refreshOptions();
                document.getElementById("count_" + this.inputId).textContent = this.items.length;
            },
...
document.getElementById("id-builder-page-1-next").addEventListener("click", page_1_next);
document.getElementById("id-builder-page-2-prev").addEventListener("click", page_2_previous);
document.getElementById("id-builder-page-2-next").addEventListener("click", page_2_next);
document.getElementById("id-builder-page-3-prev").addEventListener("click", page_3_previous);
document.getElementById("id-builder-page-3-prev").addEventListener("click", page_3_previous);
document.getElementById('build_form').addEventListener("submit", disableSubmitButton);

for (i = 0; i < 5; i++) {
    for (j = 0; j < 15; j++) {
        document.getElementById("reps_" + (i + 1) + "_" + (j + 1)).addEventListener("click", function(e){switchModeQuantity('REPS', e);});
        document.getElementById("time_" + (i + 1) + "_" + (j + 1)).addEventListener("click", function(e){switchModeQuantity('TIME', e);});
    }
}